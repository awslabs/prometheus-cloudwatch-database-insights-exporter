...
func TestBatchMetricNames(t *testing.T) {
	tests := []struct {
		...
		{
			name:                  "two complete batch one incomplete batch",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			batches := BatchMetricNames(tt.metricNames)
			...
			if tt.expectedBatches > 0 {
				...
				for i, batch := range batches {
					if i < len(batches)-1 {
						assert.Len(t, batch, 15, "all batches except the last should have size 15")
					}
				}
